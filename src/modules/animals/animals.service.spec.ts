...
const EST = '99999999-9999-9999-9999-999999999999';
...
describe('AnimalsService', () => {
  ...
  describe('changeStatus', () => {
    it('rejects an invalid transition from a terminal status', async () => {
      ...
      await expect(
        service.changeStatus(EST, 'id', { status: AnimalStatus.ACTIVE }),
    ...
    it('blocks READY_FOR_SALE while a drug-withdrawal period is active', async () => {
      ...
      await expect(
        service.changeStatus(EST, 'id', { status: AnimalStatus.READY_FOR_SALE }),
      ).rejects.toBeInstanceOf(ConflictException);
      expect(repo.changeStatus).not.toHaveBeenCalled();
    });
    ...
    it('performs a valid transition and publishes the status-changed event', async () => {
      ...
      const result = await service.changeStatus(EST, 'id', { status: AnimalStatus.QUARANTINE });
    ...
    it('allows READY_FOR_SALE when there is no active withdrawal', async () => {
      ...
      const result = await service.changeStatus(EST, 'id', { status: AnimalStatus.READY_FOR_SALE });
      expect(result.status).toBe(AnimalStatus.READY_FOR_SALE);
    });

    it('rejects acting on an animal from another establishment (404)', async () => {
      repo.findById.mockResolvedValue(buildAnimal({ establishmentId: 'other-tenant' }));
      await expect(
        service.changeStatus(EST, 'id', { status: AnimalStatus.QUARANTINE }),
  ...
  describe('findOne', () => {
    it('throws 404 when the animal does not exist', async () => {
      ...
      await expect(service.findOne(EST, 'missing')).rejects.toBeInstanceOf(NotFoundException);
    });
  });
});
