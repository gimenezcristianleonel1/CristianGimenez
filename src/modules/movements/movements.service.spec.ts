...
describe('MovementsService', () => {
  ...
  beforeEach(() => {
    ...
    locations = {
      findById: jest.fn().mockResolvedValue({ id: 'loc-to', name: 'Dest', capacity: 10 }),
  ...
  it('moves the animal and publishes the moved event', async () => {
    await service.moveAnimal('a1', { toLocationId: 'loc-to' });
  ...
  it('rejects moving into a destination at full capacity', async () => {
    ...
    await expect(service.moveAnimal('a1', { toLocationId: 'loc-to' })).rejects.toBeInstanceOf(
      ConflictException,
    );
  ...
  it('rejects moving to the same location the animal is already in', async () => {
    await expect(service.moveAnimal('a1', { toLocationId: 'loc-from' })).rejects.toBeInstanceOf(
      BadRequestException,
    );
  });
  ...
  it('throws 404 when the animal does not exist', async () => {
    ...
    await expect(service.moveAnimal('missing', { toLocationId: 'loc-to' })).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
});
